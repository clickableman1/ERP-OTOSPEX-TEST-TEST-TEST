...
export const useSettingsStore = defineStore('settings', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/settings', { data: payload });
        this.showNotification('Settings has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/settings/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Settings has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
