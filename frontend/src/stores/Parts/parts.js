...
export const usePartsStore = defineStore('parts', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`parts${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/parts', { data: payload });
        this.showNotification('Parts has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/parts/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Parts has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
