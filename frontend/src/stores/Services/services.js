...
export const useServicesStore = defineStore('services', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/services', { data: payload });
        this.showNotification('Services has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/services/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Services has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
