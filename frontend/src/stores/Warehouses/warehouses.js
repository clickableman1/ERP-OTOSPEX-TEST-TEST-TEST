...
export const useWarehousesStore = defineStore('warehouses', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`warehouses${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/warehouses', { data: payload });
        this.showNotification('Warehouses has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/warehouses/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Warehouses has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
