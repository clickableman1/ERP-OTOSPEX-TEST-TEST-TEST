...
export const useCartsStore = defineStore('carts', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/carts', { data: payload });
        this.showNotification('Carts has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/carts/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Carts has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
