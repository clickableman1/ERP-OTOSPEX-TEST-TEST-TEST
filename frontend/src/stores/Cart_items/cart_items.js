...
export const useCart_itemsStore = defineStore('cart_items', {
  state: () => ({
    data: [],
    loading: false,
    notify: {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/cart_items', { data: payload });
        this.showNotification('Cart_items has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/cart_items/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Cart_items has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
