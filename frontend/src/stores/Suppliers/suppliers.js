...
export const useSuppliersStore = defineStore('suppliers', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/suppliers', { data: payload });
        this.showNotification('Suppliers has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/suppliers/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Suppliers has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
