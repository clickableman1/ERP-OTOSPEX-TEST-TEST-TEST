...
export const useStocksStore = defineStore('stocks', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`stocks${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/stocks', { data: payload });
        this.showNotification('Stocks has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/stocks/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Stocks has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
