...
export const useSale_ordersStore = defineStore('sale_orders', {
  state: () => ({
    data: [],
    loading: false,
    notify: {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`sale_orders${query || (id ? `/${id}` : '')}`);
    ...
    async deleteItem(id) {
      try {
        await axios.delete(`/sale_orders/${id}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/sale_orders', { data: payload });
        this.showNotification('Sale_orders has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/sale_orders/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Sale_orders has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
