...
export const useNotificationsStore = defineStore('notifications', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`notifications${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/notifications', { data: payload });
        this.showNotification('Notifications has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/notifications/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Notifications has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
