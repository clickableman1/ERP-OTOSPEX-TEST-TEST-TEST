...
export const useUser_roleStore = defineStore('user_role', {
  state: () => ({
    data: [],
    loading: false,
    notify: {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`user_role${query || (id ? `/${id}` : '')}`);
    ...
    async deleteItem(id) {
      try {
        await axios.delete(`/user_role/${id}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/user_role', { data: payload });
        this.showNotification('User_role has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/user_role/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('User_role has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
