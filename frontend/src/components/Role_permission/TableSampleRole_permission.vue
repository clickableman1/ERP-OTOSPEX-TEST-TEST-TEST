...
role_permissionStore.fetch()

/* Delete item */
const deleteItem = () => {
  role_permissionStore.deleteItem(currentId.value)
}

const items = computed(() => role_permissionStore.data)

const headers = [

{ text: 'Role Id', value: 'role_id'},
{ text: 'Permission Id', value: 'permission_id'},]
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'role_id' && props.sortDirection]"
        @click="sort('role_id')"
      >Role Id</th>

      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'permission_id' && props.sortDirection]"
        @click="sort('permission_id')"
      >Permission Id</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/role_permission/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="role_id">
                {{ client.role_id }}
              </td>

              <td data-label="permission_id">
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          ...
          >
            <BaseButton
              ...
              :icon="mdiEye"
              small
              :to="`/role_permission/${client.id}/edit`"
            />
            <BaseButton
              color="danger"
              :icon="mdiTrashCan"
