...
import TableSampleCarts from '@/components/Carts/TableSampleCarts.vue'
...
const getCartsCSV = async () => {
  const response = await axios({url: '/carts?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    cartsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Carts'
    : 'http://localhost:8080/api-docs/#/Carts';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Carts</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for carts</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleCarts checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
