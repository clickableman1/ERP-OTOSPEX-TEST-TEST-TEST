...
import TableSampleRole_permission from '@/components/Role_permission/TableSampleRole_permission.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Role Id', title: 'role_id'},{label: 'Permission Id', title: 'permission_id'},
...
const getRole_permissionCSV = async () => {
  const response = await axios({url: '/role_permission?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'role_permissionCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('role_permission/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    role_permissionStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Role_permission'
    : 'http://localhost:8080/api-docs/#/Role_permission';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Role_permission</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
        label="New"
        to="/role_permission/new"
      />
      <BaseButton
        color="default"
        label="Add Filter"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for role_permission</a>

    <CardBox
      class="mb-6"
      title="Role_permission table"
      has-table
    >
      <TableSampleRole_permission checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
