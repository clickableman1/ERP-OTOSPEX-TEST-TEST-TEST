...
import TableSampleCart_items from '@/components/Cart_items/TableSampleCart_items.vue'
...
const getCart_itemsCSV = async () => {
  const response = await axios({url: '/cart_items?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    cart_itemsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Cart_items'
    : 'http://localhost:8080/api-docs/#/Cart_items';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Cart_items</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for cart_items</a>

    <CardBox
      class="mb-6"
      title="Cart_items table"
      has-table
    >
      <TableSampleCart_items checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
