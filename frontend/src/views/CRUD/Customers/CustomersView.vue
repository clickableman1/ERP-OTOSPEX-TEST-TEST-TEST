...
import TableSampleCustomers from '@/components/Customers/TableSampleCustomers.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Name', title: 'name'},{label: 'User Id', title: 'user_id'},
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    customersStore.fetch(null, query.value);
...
<CardBoxModal
  ...
  <SectionHeroBar>Customers</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for customers</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleCustomers checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
